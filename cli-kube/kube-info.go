...
func kubeInfo(namespace string, appNames []string, doPrint ...bool) string {
	var (
		wg     sync.WaitGroup
		output string
		lsOut  = make([]string, len(appNames))
	)

	wg.Add(len(appNames))
	for i, appName := range appNames {
		go func() {
			out := getInfoApp(namespace, appName)
			lsOut[i] = out
			wg.Done()
		}()
	}
...
func getInfoApp(namespace string, appName string) string {
	var (
		wg        sync.WaitGroup
